...
// POST /api/ai/generate-recipe - Generate recipe using OpenAI
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { prompt, useRealRecipe = true } = body;
    ...
    // Search the web for a single direct recipe link (only if useRealRecipe is true)
    const searchLinks = useRealRecipe ? await searchWeb(prompt) : [];
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a world-class culinary recipe generator. Create a complete, original recipe based on user requests.
...
Focus on generating a complete, original, detailed recipe. Do NOT include any links in the response.`,
        },
        {
          role: "user",
          content: prompt,
